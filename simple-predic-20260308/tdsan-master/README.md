...
datapreprocess.py: 数据预处理相关函数，包括数据分割、特征提取等
datagenerate.py: 数据生成和加载函数，包括数据集类和数据加载器
config.py: 配置参数文件，包含模型参数、训练参数等
Proposed.py: 原始的模型实现和训练脚本，主要修改的文件
LSTM_Attention_DANN.py: 新增的文件，包含LSTM+多头注意力机制与DANN网络的完整实现
loss/adv_loss.py: 实现域对抗损失函数
loss/coral.py: 实现CORAL（CORrelation ALignment）损失函数，用于域适应
主要修改的脚本和关键代码
...
创建了一个新的文件LSTM_Attention_DANN.py，包含了LSTM+多头注意力机制与DANN网络的完整实现。关键部分包括：
...
在LSTM_Attention_DANN.py中实现了完整的特征提取器
分类器改进：
...
预测精度提高：双向LSTM和多头注意力机制的结合提高了模型的特征提取能力，从而提高了预测精度。

泛化能力增强：DANN网络的集成使模型能够学习域不变的特征表示，提高了模型在不同域之间的泛化能力。

稳定性改进：批归一化和Dropout层的添加提高了模型的稳定性，减少了过拟合的风险。

适应性增强：自动域发现功能使模型能够自适应地处理不同的数据分布，提高了模型的适应性。
...
2. 配置参数 (config.py)
在运行模型前，您可以修改config.py文件中的参数来调整模型行为：
...
参数调整
您可以通过修改config.py文件中的参数来调整模型的行为：
...
自定义域标签： 如果您已经有了域标签，可以直接使用它们而不是通过GMM发现：
